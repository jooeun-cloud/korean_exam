...
EXAM_DB = {
    ...
    "고 1학년": {
        ...
        "2회차": { 1: {"ans": 1, "score": 100, "type": "테스트"} }
    },
    "고 2학년": {
        "1회차": { 1: {"ans": 1, "score": 100, "type": "테스트"} }
    },
...
**1. 진단**
현대어 풀이와 짝짓기를 못했습니다.
수능 중세 국어는 '해석 능력'이 아니라, 제시된 현대어 풀이와 옛말을 비교하는 '대응 능력'을 묻습니다.

**2. Action Plan**
1. 현대어 풀이와 일대일 대응하세요.
2. 짝을 지은 뒤 모양이 다른 부분(문법 요소)을 찾으세요.
현대어엔 '가'가 있는데 중세어엔 없다? -> 주격 조사 생략 확인.""")
...
철학 지문은 단순한 정보 전달이 아니라, 특정 화제(쟁점)에 대한 '관점의 대립'입니다.

**2. Action Plan**
1. 공통점/차이점 표를 그리세요.
지문을 눈으로만 읽지 마세요. 여백에 A vs B를 적고, 각각의 핵심 주장, 근거, 한계점을 키워드로 짧게 메모하며 읽어야 합니다.
2. 공통점과 차이점: 고난도 문제는 차이점뿐만 아니라' 두 학자가 모두 동의하는 전제(공통점)'를 물어봅니다. 
싸우면서도 동의하는 부분이 어디인지 체크하세요.""")
...
**2. Action Plan**
1. 지문 여백에 화살표 메모(`금리↑ → 투자↓`)를 통해 도식화하세요. 
2. 지문에 그래프가 나왔다면 본문을 읽기 전에 X축(가로)과 Y축(세로) 변수가 무엇인지 먼저 확인하고 동그라미 치세요. 
그래프의 기울기나 교차점은 문제로 자주 출제됩니다. 
3. <보기> 문제에서 '환율이 폭등했다' 같은 상황을 주면, 지문에서 정리한 화살표 공식에 그대로 대입해서 결과를 예측하세요. 
(지문 내용을 끼워 맞추는 것이 핵심입니다.""")

    if "과학" in question_type or "기술" in question_type:
        messages.append("""### ⚙️ [심층 분석] 과학/기술
**1. 진단**
